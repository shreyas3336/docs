---
title: "The DEX & Liquidity Engine: Powering On-Chain AI App Markets"
...
_`Governance of pool registry and protocol-fees; deploys new pair via createPair(tokenA, tokenB).`_
...
_````Core pool mechanics: mint(), burn(), swap(), sync(), and reserve queries.````_
...
_User-facing entry points: multi-hop swaps, liquidity management, permit and fee-on-transfer support._
...
_On-chain price/amount calculations by reading pair reserves along a path._
